...
class GGhostRegNet(nn.Module):
    ...
    def __init__(self, block, layers, widths, num_classes=1000, zero_init_residual=True,
        ...
        # self.sppf = SimSPPF(in_channels=widths[3], out_channels=widths[3])
...
def g_ghost_regnetx_t(**kwargs):
    return GGhostRegNet(Bottleneck, [1, 2, 7, 12], [48, 96, 192, 384], group_width=16, **kwargs)


if __name__ == '__main__':
    # TODO:C-GhostNet 用于和替换MobileNetV3的对比实验
    model = g_ghost_regnetx_t()
    model.eval()
...
#  function name here 002 represent of 200M FLOP
