...
from yolov6.layers.common import RepC2f, ACBlock
...
class VoVRep(nn.Module):
    def __init__(
            self,
            in_channels=3,
            channels_list=None,
            num_repeats=None,
            block=RepVGGBlock,  # TODO: use AC extra more spatial features
            fuse_P2=False,
            cspsppf=False
    ):
        super().__init__()
        ...
        # backbone[p1]: in/out-put size--640/320
        self.stem = block(
            in_channels=in_channels,
            out_channels=channels_list[0],
            kernel_size=3,
    ...
    def forward(self, x):
        ...
        x = self.stem(x)
        x = self.ERBlock_2(x)
        if self.fuse_P2:  # 将P2加入outputs，用于Neck学习更多细节信息
            outputs.append(x)
        x = self.ERBlock_3(x)
        outputs.append(x)
        x = self.ERBlock_4(x)
