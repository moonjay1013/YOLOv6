...
class ShuffleNetUnits(nn.Module):
    def __init__(self, in_channels, out_channels, stride, groups):
        ...
        # 如果stride = 1，mid_channels是一半，直接除以2即可  [channel split]
        else:
...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, planes, layers, num_classes=5):
        ...
        # 0.5--v6s
        # self.stage2 = self._make_layer(32, planes[0], layers[0], True)
        ...
        self.conv5 = Conv1x1BNReLU(planes[2], planes[2])

        # self.simSPPF = SimSPPF(planes[2], planes[2])
    ...
    def _make_layer(self, in_channels, out_channels, block_num, is_stage2):
        ...
        layers.append(ShuffleNetUnits(in_channels=in_channels, out_channels=out_channels, stride=2, groups=in_channels))

        # 对于stride = 1的情况，对应结构(c): 一开始就切分channel，主分支经过1x1——>3x3——>1x1再与shortcut进行concat操作
        for idx in range(block_num-1):
            layers.append(ShuffleNetUnits(in_channels=out_channels, out_channels=out_channels, stride=1, groups=in_channels*2 if is_stage2 else in_channels))
...
def shufflenet_v2_x1_5(**kwargs):
    planes = [176, 352, 704, 1048]
    layers = [4, 8, 4]
    model = ShuffleNetV2(planes, layers, 1, 1)
    return model


def shufflenet_v2_x1_0(**kwargs):  # v6s
    planes = [128, 256, 512]
    layers = [4, 8, 4]
    model = ShuffleNetV2(planes, layers, 80)
    return model


def shufflenet_v2_x0_5(**kwargs):  # v6t
    planes = [96, 192, 384]
    layers = [4, 8, 4]
    model = ShuffleNetV2(planes, layers, 80)
    return model
...
if __name__ == '__main__':
    # model = shufflenet_v2_x2_0(num_classes=80)
    # model = shufflenet_v2_x1_5(num_classes=80)
