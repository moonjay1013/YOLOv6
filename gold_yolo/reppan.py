...
from yolov6.layers.common import RepVGGBlock, RepBlock, ConvBNReLU, RepViT, ViTBlock
...
class RepGDNeck(nn.Module):
    def __init__(
        ...
        self.low_IFM = nn.Sequential(
                ...
                *[ViTBlock(extra_cfg.embed_dim_p, extra_cfg.embed_dim_p, block=block) for _ in range(1)],
                Conv(extra_cfg.embed_dim_p, sum(extra_cfg.trans_channels[0:2]), kernel_size=1, stride=1, padding=0),
        )
        
        self.reduce_layer_c5 = ConvBNReLU(
        ...
        self.Rep_p4 = RepViT(
                c1=channels_list[5],  # 256
                c2=channels_list[5],  # 256
                n=num_repeats[5],
                block=block
        )
        
        self.reduce_layer_p4 = ConvBNReLU(
        ...
        self.Inject_p3 = InjectionMultiSum_Auto_pool(channels_list[6], channels_list[6], norm_cfg=extra_cfg.norm_cfg,
                                                     activations=nn.ReLU6)
        self.Rep_p3 = RepViT(
                c1=channels_list[6],  # 128
                c2=channels_list[6],  # 128
                n=num_repeats[6],
                block=block
        )
        ...
        self.Rep_n4 = RepViT(
                c1=channels_list[6] + channels_list[7],  # 128 + 128
                c2=channels_list[8],  # 256
                n=num_repeats[7],
                block=block
        )
        ...
        self.Inject_n5 = InjectionMultiSum_Auto_pool(channels_list[10], channels_list[10],
                                                     norm_cfg=extra_cfg.norm_cfg, activations=nn.ReLU6)
        self.Rep_n5 = RepViT(
                c1=channels_list[5] + channels_list[9],  # 256 + 256
                c2=channels_list[10],  # 512
                n=num_repeats[8],
                block=block
        )
